...
export class Audition {
  static async create(audition, director_id) {
    ...
    const result = await knex("auditions")
      .where({ director_id, title: audition.title })
      .first();
    return result;
  }

  static async findAll() {
    const auditions = await knex("auditions").orderBy("created_at", "desc");
    return auditions;
  }

  static async findById(id) {
    const audition = await knex("auditions").where({ id }).first();
    return audition;
  }
  ...
  static async delete(id) {
    const result = await knex("auditions").where({ id }).del();
    return result;
  }

  static async deleteByDirectorId(director_id) {
    const result = await knex("auditions").where({ director_id }).del();
    return result;
  }
}
