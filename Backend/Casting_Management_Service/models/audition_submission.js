...
export class AuditionSubmission {
  static async create(submission) {
    await knex("audition_submissions").insert({
  ...
  static async updateStatus(id, status, directorNotes) {
    ...
    await knex("audition_submissions").where({ id }).update(updateData);

    return await knex("audition_submissions").where({ id }).first();
  }
  ...
  static async deleteByActorId(actor_id) {
    const result = await knex("audition_submissions").where({ actor_id }).del();
    return result;
  }
}
